...
const Subject = () => {
  ...
  const getAllSubject = async () => {
    try {
      const res = await api.get(`/api/subject/getSubject?page=${page}&per_page=10`);
      if (res?.status === 200) {
  ...
  const fetchResults = async (searchQuery) => {
    ...
    try {
      const response = await api.get(`/api/subject/getSubject?search=${searchQuery}&page=${page}&per_page=10`);
